...
histtimemax = 20;
plottimemax = 5000;
startdate = [2018,04,04; 2018,04,04; 2018,04,13; 2018,04,04;
    2018,04,15; 2018,03,29; 2018,04,04];


dataFolder = '/Users/baptiste/Documents/python/rPi/data/';
dbName = 'foraging.db';

monkNames = {'O','K','H','L','A','S','Q'};

if ~exist(fullfile(dataFolder,dbName),'file')
    error('Cannot find DB')
end
conn = sqlite(fullfile(dataFolder,dbName));
if conn.IsOpen
    fprintf('\nDB succesfully opened')
else
    fprintf('\nfailed to open DB')
end
...
dataSize = 0;
for mm=1:8
    sqlrequest = sprintf('SELECT id FROM recordingTable WHERE monkey=%i',mm);
    data = conn.fetch(sqlrequest);
    if ~isempty(data)
        indList = [data{:}];
        fprintf('\nMonkey %i, %i sessions found',mm,length(indList));
        dataCells{mm} = cell(length(indList),1);
        for ss=1:length(indList)
            sqlrequest = sprintf('SELECT * FROM dataTable WHERE recording=%i',indList(ss));
            data = conn.fetch(sqlrequest);
            if ~isempty(data)
                dataCells{mm}{ss} = NaN(size(data));
                dataSize  = dataSize+size(data,1);
                for cc=1:11
                    dataCells{mm}{ss}(:,cc) = double([data{:,cc}])';
                end
            end
        end
    else
        fprintf('\nMonkey %i, no sessions found',mm);
    end
    sqlrequest = sprintf('SELECT id FROM recordingTable WHERE monkey=%i',mm);
    data = conn.fetch(sqlrequest);
%     data = conn.fetch('SELECT * FROM recordingTable');
%     data = conn.fetch('SELECT * FROM recordingTable WHERE monkey=8');
end

dataMat = NaN(dataSize,11);
currData = 0;
for mm=1:8
    for ss=1:size(dataCells{mm},1)
        dataMat(currData+(1:size(dataCells{mm}{ss},1)),:) = dataCells{mm}{ss};
        currData = currData+size(dataCells{mm}{ss},1);
    end
end
...
%     
%     histbins = 21;
%     [vHist(ff,:),bHist(ff,:)] = hist(clickTimes{ff}(2:end)-clickTimes{ff}(1:end-1),linspace(0,histtimemax,histbins));
%     vHist(ff,:) = vHist(ff,:)./length(clickTimes{ff}-1);
%     
%     [vHistCorrect(ff,:),bHistCorrect(ff,:)] = hist(clickDelayCorrect{ff},linspace(0,histtimemax,histbins));
%     [vHistWrong(ff,:),bHistWrong(ff,:)] = hist(clickDelayWrong{ff},linspace(0,histtimemax,histbins));
%     [vHistWrongFirst(ff,:),bHistWrongFirst(ff,:)] = hist(clickDelayWrongFirst{ff},linspace(0,histtimemax,histbins));
...
%     subplot(1,3,1)
%     hold on
%     lc = plot(bHistCorrect(ff,:),vHistCorrect(ff,:)); lc.Color = [0,1,0,0.2];
%     lw = plot(bHistWrong(ff,:),vHistWrong(ff,:));  lw.Color = [1,0,0,0.2];
%     subplot(1,3,2)
%     hold on
...
%     hold on
%     lc = plot(bHistCorrect(ff,:),vHistCorrect(ff,:)); lc.Color = [0,1,0,0.2];
%     lw = plot(bHistWrongFirst(ff,:),vHistWrongFirst(ff,:));  lw.Color = [1,0,0,0.2];
%     subplot(1,3,2)
%     hold on
%     la = plot(cumProbWrongFirst(:,ff),cumProbCorrect(:,ff)); la.Color = [0,0,0,0.2];
%     subplot(1,3,3)
%     hold on
...
%     aaf = aucClickDelayFirst(monkList2==mm)';
...
%     aucClickDelayFirstRegr(mm,:) = (diag(ww)*[ones(size(dd)),dd])\(ww.*aaf);
...
% axis([min(trainday)-1,max(trainday)+1,0,1])
% 
% figure(14)
% hold on
% for mm=(1:7)
...
% hold on
% for mm=(1:7)
%     plot([min(trainday),max(trainday)],aucClickDelayFirstRegr(mm,1)+aucClickDelayFirstRegr(mm,2)*[min(trainday),max(trainday)],'Color',colTable(mm,:));
% end
% axis([min(trainday),max(trainday),0,1])
% xlabel('Training day')
% ylabel('AUC')
